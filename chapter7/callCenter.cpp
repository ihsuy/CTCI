...
struct customerCall
{
	...
	:caller_name(na), caller_number(nu), require_level(l),
	 resolved(false), isSpam(false), escalated(false){}
};
...
struct callCenterEmployee
{
	...
	customerCall processCall(customerCall& call)
	{
		...
		{	
			...
			if(call.require_level < 0 or call.require_level > 2)
			{	// this is spam
			...
			{	// requires higher level respondent
		...
		{	// try to let a busy respondent take another phone call
...
try 
{
	vector<string> respondent_roster
	{
		"Anne",
		"Bill",
		"Constance",
