...
struct htNode
{
	...
	htNode(const key_type& k, const val_type& v)
		: key(k), val(v){}
};
...
template<typename key_type, typename val_type>
htNode<key_type, val_type>* hashTable<key_type, val_type>::findKey(const int& index, const key_type& k)
{
	auto tar = find_if(table[index].begin(),
	                   table[index].end(),
	                   [k](htNode<key_type, val_type>* n)
	{
		return (n->key == k);
	});
...
template<typename key_type, typename val_type>
void hashTable<key_type, val_type>::insert(const key_type& k, const val_type& v)
{}
