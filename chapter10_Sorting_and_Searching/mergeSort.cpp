...
and then merges them back together. Each of those halves has the same sorting
algorithm applied to it. Eventually, you are merging just two single- element
...
right halves should be (helperLeft and helperRight). We then iterate through
helper, copying the smaller element from each half into the array. At the end,
...
void merge(vector<int>& v,
           ...
           int mid,
           int high,
           vector<int>& v_copy) {  // note here, no new container is created
...
void mergeSort_helper(vector<int>& v,
                      const int& left,
                      const int& right,
                      vector<int>& v_copy) {
    if (left >= right) {
