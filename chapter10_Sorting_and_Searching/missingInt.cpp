...
int findMissingInt_1GB(const vector<unsigned long long>& v /*10m non-negative integers*/)
{
	bitset<10000000> bs;
...
int findMissingInt_10MB(const vector<unsigned long long>& v /*10m non-negative integers*/)
{
	...
	{
		if(rangeCount[rc] < 10000)
		{	// missing int found here
			break;
		}
	}
...
int main()
{
	...
	shuffle(v.begin(), v.end(), default_random_engine((int)chrono::high_resolution_clock::now().time_since_epoch().count()));
