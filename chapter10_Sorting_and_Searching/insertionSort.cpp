...
again doing a linear scan. Continue doing this until all the elements are in
place.
*/
...
void insertionSort2(
    vector<int>& v) {  // maintain a sorted array in the head of v
    // constantly insert next element to the correct location in the sorted
    // array
    for (int cur = 1; cur < v.size();
         ++cur) {  // insert v[cur] to the sorted subarray before cur
        int sorted_i = cur - 1;
        while (sorted_i >= 0 and
               v[sorted_i + 1] <
                   v[sorted_i]) {  // swap v[cur] to the front and move every
                                   // elements to their position + 1
...
int main() {
    ...
    timeIt(insertionSort2, "insertionSort2: insert next to correct position",
           v2);
