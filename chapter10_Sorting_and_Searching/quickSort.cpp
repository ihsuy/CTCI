...
a series of swaps (see below). If we repeatedly partition the array (and its
sub-arrays) around an element, the array will eventually become sorted. However,
as the partitioned element is not guaranteed to be the median (or anywhere near
the median), our sorting could be very slow.This is the reason for the 0 (n2)
worst case runtime.
*/
...
void quickSort_helper(vector<int>& v, const int& low, const int& high) {
    // not sure why embedding insertion sort here slows down the algorithm
    // significantly since insertion sort is often faster when sorting an array
    // with < 10 elements but it doesn't seem to work when combined with quick
    // sort
