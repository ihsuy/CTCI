...
each digit. For example, if we have an array of integers, we might first sort by
the first digit, so that the Os are grouped together. Then, we sort each of
these groupings by the next digit. We repeat this process sorting by each
subsequent digit. until finally the whole array is sorted. Unlike comparison
sorting algorithms, which cannot perform better than 0 (n log (n)) in the
average case, radix sort has a runtime of O( kn), where n is the number of
elements and k is the number of passes of the sorting algorithm.
*/

void radixSort_helper(
    vector<int>& v,
    const int& n /*nth digit to sort with respect to*/) {  // create buckets 10
...
void radixSort(vector<int>& v) {
    ...
    for (int n = 0; n < nd;
         ++n) {  // since radixSort_helper does an inplace sorting on the array
