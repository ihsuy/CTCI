...
void sortedMerge1(vector<int>& a, vector<int>& b) {  // use an extra array
    vector<int> e(a.size());
...
void sortedMerge2(vector<int>& a,
                  const int& a_size,
                  vector<int>& b) {  // in place
    // trick is to insert to the back of a
    int k = a.size() - 1;
    int i = a_size - 1, j = b.size() - 1;

    while (j >= 0) {  // terminate only when we finish iterating b
        if (i >= 0 and
            a[i] > b[j]) {  // won't enter here if i is exhausted first
            a[k] = a[i];
            i--;
        } else {
            a[k] = b[j];
