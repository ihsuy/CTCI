...
void peakAndValleys_optimal(vector<int>& v) {
    ...
    // the last number in the past 3 numbers (already in peak_and_valley form)
    // call it n there're 2 possibilities: n is a valley or n is a peak For case
    // 1: its only problematic when the next number, let's call it m, is less
    // than n solution will be to simply swap n and m. Since m is less than n, m
    // is qualified for being the valley at n's position, and if we swap them, m
    // is perfectly fine, and n is larger than m so becomes peak and its fine
    // too For case 2: exactly same approach can be applied. Since there's only
    // trouble when next number m is larger than n, we can swap m and n, then m
    // is the new peak and n becomes the valley next to it.

    // By applying the same rules, by induction, we can transform the input
    // array to a peak_and_valley sequence without sorting

    for (int i = 1; i < v.size() - 1; ++i) {
        int h = v[i - 1], n = v[i], m = v[i + 1];
        if ((h > n and m < n) or (h < n and m > n) or
            (h == n)) {  // n is valley that's followed by m, m<n
...
int main() {
    vector<int> v{200, 200, 1300, 180, 311, 3,   2,  10,
                  12,  0,   3,    4,   99,  190, 200};
