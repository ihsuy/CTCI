...
struct person
{
	...
	person(const int& init_nfriends = 10): id(global_id) {global_id++; friends.reserve(init_nfriends);}

};

vector<person*> createSocialNetworks(const int& nUser /*total number of user in the network*/,
                                     const int& nfriends /*number of friends each user has*/)
{
	...
	for (int i = 0; i < nUser; ++i)
	{
		...
		{
			if (find(users[i]->friends.begin(), users[i]->friends.end(), users[idx[c]]) == users[i]->friends.end())
			{
		...
		for (int j = 0; j < fs; ++j)
		{
			if (find(users[i]->friends[j]->friends.begin(), users[i]->friends[j]->friends.end(), users[i]) == users[i]->friends[j]->friends.end())
			{
...
vector<int> getPath(const unordered_map<int, int>& m1, const unordered_map<int, int>& m2,
                    const int& p1, const int& p2, const int& mid)
{
	vector<int> path1;
	vector<int> path2;
	cout << mid << endl;
