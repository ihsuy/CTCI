...
pair<int, int> smallestDifference_helper(
    const vector<int>& v1,
    const vector<int>& v2) {  // v1 and v2 are supposed to be sorted and for
    ...
    for (int i = 0; i < v1.size();
         ++i) {  // binary search every element of v1 in v2
        ...
        while (low >= high) {
            ...
            if (v2[mid] == current) {  // found 0 difference,
                // its ofc the answer
...
pair<int, int> smallestDifference(
    vector<int> v1,
    vector<int> v2) {  // sort so we can do binary search
    ...
    // if its not found, the answer must be one element before the stopping
    // position or one element after it (if exist)
