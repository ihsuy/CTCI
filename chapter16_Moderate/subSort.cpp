...
minimize n-m
*/
bool isInCorrectLocation(const vector<int>& v,
                         const int& idx) {  // partition v using v[idx] as pivot
...
pair<int, int> subSort_optimal(const vector<int>& v) {
    ...
    if (left_end == v.size() - 1) {  // v is already sorted
        return {0, 0};
    }
...
int main() {
    ...
    cout << "subSort_naive: (" << result1.first << ", " << result1.second << ")"
         << endl;
    auto result2 = subSort_optimal(v);
    cout << "subSort_naive: (" << result2.first << ", " << result2.second << ")"
         << endl;
    return 0;
}
