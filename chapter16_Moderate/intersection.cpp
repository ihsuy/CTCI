...
pair<double, double> line_intersection(const pair<double, double>& p11,
                                       const pair<double, double>& p12,
                                       const pair<double, double>& p21,
                                       const pair<double, double>& p22) {
    ...
    if (k1 ==
        k2) {  // parallel -> no intersection unless they are the same line
        if (b1 == b2) {  // same line
...
pair<double, double> lineSegments_intersection(
    ...
    if (x >= line1_x_low and x <= line1_x_high and x >= line2_x_low and
        x <= line2_x_high and y >= line1_y_low and y <= line1_y_high and
...
int main() try {
    ...
    pair<double, double> intersection =
        lineSegments_intersection(p11, p12, p21, p22);

    cout << "intersection at: (" << intersection.first << ", "
         << intersection.second << ")\n";
