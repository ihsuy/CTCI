...
void possibleBoardLength_helper(const int& k, const int& b1, const int& b2, const int& len, set<int>& result, unordered_set<string>& alreadyGenerated)
{
	if (k == 0)
	{
		result.insert(len);
	...
	possibleBoardLength_helper(k - 1, b1, b2, len + b1, result, alreadyGenerated);
	possibleBoardLength_helper(k - 1, b1, b2, len + b2, result, alreadyGenerated);

	alreadyGenerated.insert(key);
}

set<int> possibleBoardLength_recursive(const int& k, const int& longer, const int& shorter)
{	// extremely slow
	set<int> result;
...
vector<int> possibleBoardLength(const int& k, const int& longer, const int& shorter)
{
	if (longer == shorter)
	{
		return {longer * k};
