...
struct trieNode {
    ...
    char val;
    bool isWordEnd;
    trieNode(const char& c, const bool& iwe = false)
        : children(26, nullptr), val(c), isWordEnd(iwe) {}
};
...
bool wordFound(trieNode* root, const string& word) {
    ...
    int alphval = chtoi(word[i]);
    if (root->children[alphval] != nullptr and
        root->children[alphval]->isWordEnd) {
        return true;
    }
    return false;
}
...
void extra_match(const string& letters,
                 const int& depth,
    ...
    if (depth != default_depth and cur_node != nullptr and
        cur_node->isWordEnd) {
    ...
    for (int i = 0; i < 26; ++i) {
        extra_match(letters + (char)('a' + i), depth - 1, default_depth,
                    engWords, suggestions);
    }
}
...
void matchWords(
    ...
    const int& extra_depth) {  // userInput shouldn't be empty string
    ...
    if (idx == userInput.length()) {
        ...
        } else {  // if its not word end, suggest slightly longer words
            extra_match(letters, extra_depth, extra_depth, engWords, result);
        }
        return;
    }
    ...
    for (int i = 0; i < choices.length(); ++i) {
        matchWords(userInput, idx + 1, letters + choices[i], engWords, result,
                   extra_depth);
    }
}

vector<string> autoSuggestion(const string& userInput,
                              trieNode* engWords,
                              const int& extra_depth = 3) {
...
int main() {
    auto engWords =
        loadEnglishWords("/Users/ihsuy/crack/chapter16_Moderate/engWords.txt");
