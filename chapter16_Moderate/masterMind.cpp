...
YRGB. When you guess the correct color for the correct slot, you get a "hit:' If
you guess a color that exists but is in the wrong slot, you get a "pseudo-hit:'
Note that a slot that is a hit can never count as a pseudo-hit. For exam ple, if
the actual solution is RGBY and you guess GGRR, you have one hit and one pseudo
-hit. Write a method that, given a guess and a solution, returns the number of
...
pair<int, int> masterMind(const string& solution, const string& guess) {
    ...
    for (int i = 0; i < solution.length();
         ++i) {  // if guess is correst increase hitt
        if (solution[i] == guess[i]) {
            hit++;
        } else {  // if its not correct store letter count for later use
            // to count pseudo hit
    ...
    // count pseudo_hit
    for (int i = 0; i < solution.length(); ++i) {  // if it's not a real hit
        // and it has appeared at least once in solution except when it was
        // correct
