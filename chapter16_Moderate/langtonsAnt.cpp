...
(clockwise), and move forward one unit. (2) At a black square, flip the color of
the square, turn 90 degrees left (counter-clockwise). and move forward one unit.
...
represent the grid. This is something you must design yourself. The only input
to your method is K. You should print the final grid and return
nothing.Themethodsignaturemightbesomethinglikevoid printKMoves(int K).
*/

enum marker { BLACK_SQUARE = '#', WHITE_SQUARE = ' ' };
...
void ant_progress(vector<string>& grid,
                  int& cur_r,
                  int& cur_c,
                  directions& dir) {
...
void langtonsAntSimulator(
    const int& width,
    const int& height,
    const int& speed =
        1) {  // set default initial position to middle of the grid
