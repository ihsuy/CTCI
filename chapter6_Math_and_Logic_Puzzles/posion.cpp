...
bool testingStrip::get_result() {
    if (days_elapsed < 7) {
        throw runtime_error("Please wait! Result will be out in " +
                            to_string(7 - days_elapsed) + " days!");
    } else {
        return isPositive;
    }
}
...
void FindOutPosionBottle(const vector<bool>& bottles,
                         vector<testingStrip>& strips) {
    ...
    for (int i = 0; i < strips.size(); ++i) {
        cout << "TestingStrip #" << i + 1
             << ((strips[i].get_result()) ? "  Postive" : "  Negative") << endl;
    }

    // consider the result as the binary representation of the target bottle
    // index convert that index to decimal
    ...
    cout << "Experiment result : Posion is in the "
         << bitset<10>(bstr2).to_ulong() << "th bottle" << endl;
