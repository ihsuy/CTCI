...
void apocalypse_simulation(const int& nFamily, const int& nExp)
{
	...
	for(int i = 0; i < nExp; ++i)
	{
		...
		cout << "Experiment #" << i << "  b: " << temp_boys << "  g: " << nFamily << endl;
		boys += temp_boys;
	}

	cout << nExp << " simulations completed." << endl;
	cout << "Average result:\nb: " << (double)boys/nExp << "  g: " << nFamily << endl;
}
...
void natrual_simulation(const int& nFamily, const int& nExp)
{
	...
	for(int i = 0; i < nExp; ++i)
	{
		...
		cout << "Experiment #" << i << "  b: " << temp_boys << "  g: " << temp_girls << endl;
		boys += temp_boys;
		girls += temp_girls;
	}

	cout << nExp << " simulations completed." << endl;
	cout << "Average result:\nb: " << (double)boys/nExp << "  g: " << (double)girls/nExp << endl;
}
