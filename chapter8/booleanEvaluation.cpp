...
int booleanEvaluation_helper(const string& s, const char& cur_val, const int& front,
                             const bool& desiredValue)
{
	...
	cout << "cur_val: " << cur_val << " front: " << front << " op: " << op << " dv: " << desiredValue <<'\n';

	if(cur_val == '1')
	{
		if(op == '&')
		{
			result += booleanEvaluation_helper(s, front_val, front+2, desiredValue);
		}
		...
		{
			result += booleanEvaluation_helper(s, front_val, front+2, not desiredValue);
		}
	}
	else
	{
	...
	result += booleanEvaluation_helper(s, temp_result+'0', front+2, desiredValue);
	cout << "result2: " << result << '\n';
	return result;
