...
struct tStack {
    ...
    int a_bot, b_bot, c_bot, a_top, b_top, c_top;
...
int tStack::a_pop() {
    if (a_top ==
        a_bot + 1) {  // stack is empty, throw an error (for realistic senario)
...
int tStack::b_pop() {
    if (b_top ==
        b_bot + 1) {  // stack is empty, throw an error (for realistic senario)
...
int tStack::c_pop() {
    if (c_top ==
        c_bot + 1) {  // stack is empty, throw an error (for realistic senario)
...
void tStack::a_push(const int& val) {
    if (a_top == b_bot) {  // exceeded capacity, double A's size
        cout << "A capacity exceeded, reallocating space... " << a_size << "->"
             << a_size * 2 << endl;
...
void tStack::info() {
    ...
    cout << setw(25) << "A size: " << setw(25) << a_top - 1 << setw(25)
         << "from: " << a_bot << endl;
    cout << setw(25) << "B size: " << setw(25) << b_top - b_bot - 1 << setw(25)
         << "from: " << b_bot << endl;
    cout << setw(25) << "C size: " << setw(25) << c_top - c_bot - 1 << setw(25)
         << "from: " << c_bot << endl;
