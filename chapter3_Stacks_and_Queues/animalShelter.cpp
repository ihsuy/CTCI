...
{
	if ((cats.size() == 0 or dogs.front().id < cats.front().id) and (dogs.size() != 0))
	{
		dequeueDog();
	}
	else if ((dogs.size() == 0 or dogs.front().id > cats.front().id) and (cats.size() != 0))
	{
		dequeueCat();
	}
