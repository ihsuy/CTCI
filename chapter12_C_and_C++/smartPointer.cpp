...
class smartPointer {
   ...
   public:
    smartPointer(T p) : ptr(p) { refCounter[p]++; };

    void copy(const smartPointer<T>& obj) {  // copy another smartPointer obj
        if (obj.ptr !=
            ptr) {  // if this smartPointer doesn't point to the same object
            refCounter[ptr]--;
        } else {  // if its the same obj
            refCounter[ptr]++;
        }

        if (refCounter[ptr] ==
            0) {  // copy may cause reference count for certain obj becoming 0
...
int main() {
    ...
    name3.copy(name1);  // refCounter for aname -- (1->0): this will cause space
                        // held by aname to be released

    string* hname = new string("hisaishi");
    name1 = hname;  // refCounter for yname -- (2->1)
    name2 = hname;  // refCounter for yname -- (1->0):  this will cause space
                    // held by yname to be released
