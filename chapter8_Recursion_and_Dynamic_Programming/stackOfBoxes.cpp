...
int stackOfBoxes_helper(const vector<box>& boxes,
                        const int& top,
    ...
    for (int i = top + 1; i < boxes.size(); ++i) {
        if (boxes[i] > boxes[top]) {
            ...
            if (buffer.find(top) != buffer.end() and
                buffer[top].find(i) != buffer[top].end()) {
                temp_h = buffer[top][i];
            } else {
...
int stackOfBoxes(vector<box>& boxes) {
    ...
    sort(boxes.begin(), boxes.end(),
         [](const box& b1, const box& b2) { return b1.h < b2.h; });
...
int main() {
    vector<box> boxes{
        box(3, 5, 3), box(8, 8, 4), box(2, 2, 2), box(1, 1, 1), box(10, 9, 10),
    };
...
/*		auto removedBox = *(boxes.begin()+top);
                cout << "removed: " << removedBox.w << " " << removedBox.h << "
   " << removedBox.d << '\n'; boxes.erase(boxes.begin()+top); cout << "box: ";
                for(auto& item: boxes)
                {
