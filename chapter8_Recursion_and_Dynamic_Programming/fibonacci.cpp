...
long long fibonacci_handler(
    const int& n,
    long long (&cache)[upb + 1]) {  // calculate n-th fibonacci number
    if (n <= 0) {
        return 0;
    } else if (n == 1) {
        return 1;
    ...
    if (cache[n] == 0) {
        result =
            fibonacci_handler(n - 1, cache) + fibonacci_handler(n - 2, cache);
...
int main() {
    ...
    auto time1 =
        (double)chrono::duration_cast<chrono::microseconds>(t2).count() / 1e6;
