...
void permWithoutDups_helper(
    string& s,
    const int& i,
    const int& j) {  // this method originate from the observation that
    ...
    for (int si = i; si <= j;
         ++si) {  // the loop and strSwap are replacing the 'head' character
...
vector<string> permWithoutDups2(
    string s) {  // build from the previous n-1 permutations
