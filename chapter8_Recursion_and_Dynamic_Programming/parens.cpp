...
void parens_helper(const int& lb,
                   const int& rb,
                   const string& prefix) {  // lb: number left brackets; rb:
                                            // number right brackets
    if (lb == 0 and rb == 0) {  // when there's no more brackets left
        // we've completed prefix
        cout << prefix << '\n';
    } else if (lb == 0) {
        parens_helper(lb, rb - 1, prefix + rightBracket);
    } else if (lb ==
               rb) {  // when we have same amount of lb and rb on our hands
        // we can only place lb otherwise we will have 1 extra rb
        // that's not offset
        parens_helper(lb - 1, rb, prefix + leftBracket);
    } else {  // otherwise we could choose to append either one
        parens_helper(lb - 1, rb, prefix + leftBracket);
        parens_helper(lb, rb - 1, prefix + rightBracket);
    }
}
