...
void permWithDups_helper(map<char, int>& freqCounter,
                         const int& remain,
                         const string& prefix) {
    if (remain == 0) {  // nothing left to be added
        // prefix is completed
        cout << prefix << endl;
        return;
    }
    ...
    for (auto& item :
         freqCounter) {  // for every char-int pair in the freqCounter
        // look for those has at least one char left in freqCounter
        // append each of them to prefix and recurse
        if (item.second >
            0) {  // item counter should be decremented since it's about to be
            // appended to prefix
            item.second--;
            permWithDups_helper(freqCounter, remain - 1,
                                prefix + string(1, item.first));
