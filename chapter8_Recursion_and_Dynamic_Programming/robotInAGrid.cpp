...
enum MOVE
{
	RIGHT, DOWN, NULL_MOVE
};

enum gridMark
{
	EMPTY, OBSTACLE, BEEN_HERE
};

void deployObstacle(vector<vector<gridMark>>& grid, const vector<pair<int, int>>& loc)
{
...
vector<MOVE> findRoute(vector<vector<gridMark>>& grid)
{	// iterative
	vector<MOVE> route;
	...
	{
		if ((x != grid[0].size() - 1 and y != grid.size() - 1) and grid[y][x + 1] == 1 and grid[y + 1][x] != 1)
		{
			// paint route on grid
		...
		else if ((x != grid[0].size() - 1 and y != grid.size() - 1) and grid[y][x + 1] != 1 and grid[y + 1][x] == 1)
		{
			grid[y][x] = gridMark::BEEN_HERE;
		...
		else if (grid[y][x] == gridMark::EMPTY and (x != grid[0].size() - 1 and y != grid.size() - 1) and grid[y][x + 1] != 1 and grid[y + 1][x] != 1)
		{
...
bool findRoute_helper(vector<vector<gridMark>> & grid, 
	const int& r, const int& c, vector<vector<gridMark>>& path,
	vector<vector<int>>& cache)
{
	...
	if(atStartingPoint or findRoute_helper(grid, r-1, c, path, cache) 
		or findRoute_helper(grid, r, c-1, path, cache))
	{
...
{
	vector<vector<int>> cache(grid.size(),
	                           vector<int>(grid[0].size(), 0));

	vector<vector<gridMark>> path(grid.size(),
	                           vector<gridMark>(grid[0].size(), gridMark::EMPTY));
...
void createObstacle(const int& r, const int& c, const int& n, vector<pair<int, int>>& obstacles)
{
