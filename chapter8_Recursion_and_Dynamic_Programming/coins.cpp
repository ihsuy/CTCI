...
int makeChange2_helper(const int& money, const bool& useQ, const bool& useD,
                       const bool& useN)
{
	if (money < 0)
	{
...
int makeChange_helper(const int& money, const vector<int>& coins, const int& index,
                      unordered_map<int, unordered_map<int, int>>& buffer)
{
	...
	for (int i = 0; i * coins[index] <= money; ++i)
	{
		...
		if (buffer.find(index) != buffer.end() 
			and buffer[index].find(moneyLeft) != buffer[index].end())
		{
