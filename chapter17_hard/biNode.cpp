...
void udpate_and_connect(BiNode*& prev_node,
                        BiNode*& current_node,
                        BiNode* new_node,
                        BiNode*& dll) {  // update prev_node with current_node
    ...
    if (prev_node !=
        nullptr) {  // be careful when prev_node is nullptr which means
        // we are at the head of the result double linkedlist
        prev_node->right = current_node;
    } else {  // when prev_node is nullptr we know current_node is the smallest
              // value
...
BiNode* bst_to_doubleLinkedList(BiNode* root) {
    ...
    while (not todo.empty()) {
        if (todo.top()->left != nullptr) {
            ...
            while (not todo.empty() and
                   todo.top()->right ==
                       nullptr) {  // connect nodes of bst in place
                // since we are at our current_node position now
                // we don't care if we mess with the links on the left hand side
                // of it since we are done with any nodes on the left hand side
                // of it
