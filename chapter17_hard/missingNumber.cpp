...
bool getBit(const vector<int>& A,
            const int& i,
            const int& j) {  // get the j-th bit of A[i]
    return ((A[i] & (1 << j)) != 0);
}
...
vector<int> bitSum(
    int n) {  // calculate correct sum of each bit respectively from 0-n
    vector<int> result(32, 0);

    for (int mod = 2, count = 0, block = n / mod; block != 0;
         count++) {  // pattern can be found after careful observation
        int remainder = n % mod;
