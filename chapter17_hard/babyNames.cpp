...
vector<pair<string, int>> mergeNameFrequencies(
    const vector<pair<string, int>>& names,
    const vector<pair<string, string>>&
        synonyms) {  // use a hashTable for quick look up of number of person
                     // with certain name
    ...
    // since saying A is synonymous B is equivalent to saying B is synonymous to
    // A
    ...
    // to avoid counting the same names again, as well as detecting loops if
    // exists
    ...
    for (auto& connections :
         graph) {  // only proceed if we've never seen this name so far
        ...
        pair<string, int> tmp{connections.first,
                              number_of_person.count(connections.first) == 0
                                  ? 0
                                  : number_of_person[connections.first]};
        ...
        while (not todo.empty()) {
            ...
            if (graph.count(p) !=
                0) {  // if the name doesn't exist in the first field of the
                // key_value pairs of the map, its a dead end
                for (auto& name :
                     graph[p]) {  // check if seen to avoid infinite loop
...
int main() {
    vector<pair<string, int>> name_info{
        {"John", 15},        {"Jon", 12},  {"Chris", 13}, {"Kris", 4},
        {"Christopher", 19}, {"Yushi", 1}, {"Yuushi", 3},
    };
