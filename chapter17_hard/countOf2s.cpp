...
// brute force
int count2s(int n) {  // count number of 2s in an integer
...
int count2s_under_bf(
    const int& n) {  // brute forcely count 2s in each number and sum them up
    int result = 0;
    for (int i = 2; i <= n; ++i) {
        result += count2s(i);
    }
...
// Solution
int count2s_forPowerOf10_under(
    const int& n) {  // only works for positive powers of 10
    ...
    // 				 namely from 2xxx... to 2999... and this range
    // itself 				 contributes 10^N much of 2s.
    if (n == 0) {
        return 0;
    }
    if (n == 10) {
...
int count2s_under(const int& n, unordered_map<int, int>& buffer) {
    ...
    if (sig >
        2) {  // if we have more than 2 bases we have 1 base much of extra 2s
        // (explained in count2s_forPowerOf10_under function)
        result *= sig;
        result += base;
    } else if (sig == 2) {  // if we have exactly 2 bases then
        // we need to handle the "uncomplet special range"
        result *= 2;
        // contrast to the case above which adds a whole base to the result
        // we add exactly remainder + 1 much of extra 2s since the ranges isn't
        // complete
        result += remainder + 1;
    }
