...
class circularArray {
   ...
   public:
    ...
    iterator begin() {
        return iterator(&internal_data[head_pos], internal_data,
                        &internal_data[size], size);
    }
    iterator end() {
        return iterator(&internal_data[(head_pos + size) % size], internal_data,
                        &internal_data[size], size);
    }
...
template <typename T>
circularArray<T>::circularArray(const initializer_list<T>& l) : head_pos(0) {
...
class circularArray<T>::iterator {
   ...
   public:
    ...
    bool operator==(const iterator& ite) const { return (ptr == ite.ptr); }
