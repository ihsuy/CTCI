...
struct CD {
    ...
    CD(const string& t, const string& s) : title(t), song(s) {}
};
...
struct CDbox {
    ...
    CDbox(const vector<CD>& CDs) {
        if (CDs.size() > capacity) {
            throw runtime_error("Too many CDs. CDbox can only hold up to " +
                                to_string(capacity) + " CDs");
        }
    ...
    vector<string> get_CDtitles() { return titles; }
    ...
    void say(const string& msg) { cout << "CDbox: " << msg << endl; }
};
...
struct userInterface {
    ...
    userInterface(const vector<string>& t)
        : titles(t),
          money_box(0),
          paid(false),
          userChoice(0),
    ...
    int get_choice() { return userChoice; }

    void say(const string& msg) { cout << "userInterface: " << msg << endl; }
};
...
struct jukebox {
    ...
    jukebox(const vector<CD>& CDs)
        : spkr(new speaker()),
          cdb(new CDbox(CDs)),
          ui(new userInterface(cdb->titles)) {}
...
int main() {
    vector<CD> CDs{CD("Sign", "todoite kurerunokana"),
