...
map<char, unordered_set<char>> buildDependencyMap(const vector<char>& projects, const vector<dependency>& deps)
{
...
vector<char> buildOrder(vector<char> projects, vector<dependency> dependencies)
{
			...
			if(item.second.size() == 0)
			{
				...
				for(auto& item2 : dm)
				{
					if((item2.first != item.first) 
						and (item2.second.find(item.first) != item2.second.end()))
					{
						       ...
						       const vector<string>& dependencies)
{
	...
	{	
		graph[dependencies[i][0]]->outgoings.push_back(graph[dependencies[i][1]]);
