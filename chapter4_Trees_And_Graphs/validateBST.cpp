...
int BSTmax(node* root) {  // assume BST structure and find its max element
    if (root == nullptr) {
        return INT_MIN;
    }
...
int BSTmin(node* root) {  // assume BST structure and find its max element
    if (root == nullptr) {
        return INT_MAX;
    }
...
bool isBST2(node* root, const int& mi, const int& ma) {
    ...
    return (isBST2(root->left, mi, root->val) and
            isBST2(root->right, root->val, ma));
}

int main() {
    // create a tree
    node* tree =
        createBinaryTree(vector<int>{15, 6, 17, 4, 8, 16, 19, 4, 7}, 0);
