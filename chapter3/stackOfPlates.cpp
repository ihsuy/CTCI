...
struct SetOfStacks
{
	...
	int current_height;
	SetOfStacks(const int& mh): stacks(new node * [1]), stacks_size(1), max_height(mh), current_height(0) {}
...
void SetOfStacks::popAt(const int& index)
{
	...
	{
		throw SetOfStack_exception((to_string(index) + "th stack doesn't exist.").c_str());
	}
