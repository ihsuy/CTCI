...
char tictacWin_3x3_naive(const vector<vector<char>>& grid)
{	// return winner's mark: 2 players: 'O' and 'X'
	...
	if (x_count1 == 3 or o_count1 == 3 or
	        x_count2 == 3 or o_count2 == 3)
	{
...
char tictacWin_3x3_cleaner(const vector<vector<char>>& grid)
{
	...
	if (threeEqual(grid[0][0], grid[1][1], grid[2][2])
	        or threeEqual(grid[0][2], grid[1][1], grid[2][0]))
	{	// since two diagonals intersect at grid[1][1]
		return grid[1][1];
	}
...
bool rowCompleted(const vector<vector<char>>& grid, const int& r, const int& width)
{
...
bool colCompleted(const vector<vector<char>>& grid, const int& c, const int& width)
{
	for (int i = 1; i < width; ++i)
	{
